...
export class WebGPURenderer {
  ...
  private async createPipelines(): Promise<void> {
    ...
    // Uniform buffer (time, resolution, mapProgress)
  ...
  private render(): void {
    ...
    const bgPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.96, g: 0.91, b: 0.76, a: 1 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
  ...
  emitPiecePickup(x: number, y: number, landType: string): void {
    this.particleSystem.emitPiecePickup(x, y, landType);
  }

  emitPiecePlaced(x: number, y: number, landType: string): void {
    this.particleSystem.emitPiecePlaced(x, y, landType);
  }
