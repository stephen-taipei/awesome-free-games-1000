/**
 * WebGPU Renderer - Matchstick Puzzle
...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    const backgroundPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        ...
        loadOp: 'clear',
        storeOp: 'store',
        clearValue: { r: 0.1, g: 0.05, b: 0.02, a: 1 },
      }],
    });
  ...
  emitEmber(x: number, y: number): void {
    this.particleSystem.emitEmber(x, y);
  }
  ...
  emitSmoke(x: number, y: number): void {
    this.particleSystem.emitSmoke(x, y);
  }
