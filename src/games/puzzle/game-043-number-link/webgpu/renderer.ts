...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    if (this.victoryIntensity > 0) {
      this.victoryIntensity = Math.max(0, this.victoryIntensity - deltaTime * 0.4);
    }
    ...
    const backgroundPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        ...
        loadOp: 'clear',
        storeOp: 'store',
        clearValue: { r: 0.02, g: 0.01, b: 0.06, a: 1 },
      }],
    });
