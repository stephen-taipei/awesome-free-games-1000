...
export class WebGPURenderer {
  ...
  // Vine grows to new tile
  emitVineGrow(x: number, y: number): void {
    this.particles.emit(x, y, 'sprout', 5);
    this.particles.emit(x, y, 'leaf', 2);
  }

  // Water tile boost
  emitWaterBoost(x: number, y: number): void {
    this.particles.emit(x, y, 'water', 8);
    this.particles.emit(x, y, 'sprout', 3);
  }

  // Sun tile boost
  emitSunBoost(x: number, y: number): void {
    this.particles.emit(x, y, 'sunbeam', 5);
    this.particles.emit(x, y, 'pollen', 4);
  }

  // Flower blooms
  emitFlowerBloom(x: number, y: number): void {
    this.particles.emit(x, y, 'bloom', 15);
    this.particles.emit(x, y, 'pollen', 10);
    this.particles.emit(x, y, 'sunbeam', 4);
  }
  ...
  emitVictory(): void {
    ...
    for (let i = 0; i < 8; i++) {
      setTimeout(() => {
        ...
        this.particles.emit(x, y, 'bloom', 12);
        this.particles.emit(x, y, 'leaf', 6);
        this.particles.emit(x, y, 'pollen', 8);
      }, i * 100);
    }

    // Final burst
    setTimeout(() => {
      this.particles.emit(centerX, centerY, 'bloom', 20);
      this.particles.emit(centerX, centerY, 'sunbeam', 8);
    }, 900);
  }
  ...
  emitLevelStart(): void {
    for (let i = 0; i < 15; i++) {
      ...
      this.particles.emit(x, y, 'pollen', 1);
    }
    for (let i = 0; i < 5; i++) {
      const x = Math.random() * this.canvas.width;
      const y = Math.random() * this.canvas.height * 0.3;
      this.particles.emit(x, y, 'leaf', 1);
    }
  }

  // Tile click feedback
  emitTileClick(x: number, y: number): void {
    this.particles.emit(x, y, 'leaf', 2);
    this.particles.emit(x, y, 'pollen', 3);
  }
