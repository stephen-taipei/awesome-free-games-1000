...
export class WebGPURenderer {
  ...
  private telescopeX: number = 0.5;
  private telescopeY: number = 0.5;
  ...
  // Event emitters
  updateTelescopePosition(x: number, y: number, radius: number): void {
    this.telescopeX = x;
    this.telescopeY = y;
    this.viewRadius = radius;

    // Emit lens particles around telescope
    if (Math.random() < 0.3) {
      this.particleSystem.createParticle(x, y, 'lens');
    }
  }

  emitStarDiscovery(x: number, y: number): void {
    this.particleSystem.emitBurst(x, y, 'discovery', 20, ASTRO_COLORS.starGold);
    this.particleSystem.emitRing(x, y, 0.05, 'star', 12, ASTRO_COLORS.starWhite);
    this.particleSystem.emitBurst(x, y, 'lens', 8);
  }

  emitTelescopeMove(x: number, y: number): void {
    if (Math.random() < 0.2) {
      this.particleSystem.createParticle(x, y, 'cosmic');
    }
  }
  ...
  emitVictory(): void {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      this.particleSystem.createParticle(x, y, 'discovery', ASTRO_COLORS.starGold);
    }
    for (let i = 0; i < 30; i++) {
      this.particleSystem.createParticle(Math.random(), Math.random(), 'star');
    }
  }
  ...
  emitLevelStart(): void {
    ...
    for (let i = 0; i < 15; i++) {
      this.particleSystem.createParticle(Math.random(), Math.random(), 'cosmic');
    }
    for (let i = 0; i < 8; i++) {
      this.particleSystem.createParticle(Math.random(), Math.random(), 'nebula');
    }
  }
  ...
  emitAmbient(): void {
    // Occasional cosmic particles
    if (Math.random() < 0.1) {
      this.particleSystem.createParticle(Math.random(), Math.random(), 'cosmic');
    }
    // Rare shooting star
    if (Math.random() < 0.02) {
      this.particleSystem.createParticle(0, Math.random() * 0.5, 'comet');
    }
  }
