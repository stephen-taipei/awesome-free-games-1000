/**
 * WebGPU Renderer - Submarine Puzzle
...
export class WebGPURenderer {
  ...
  private submarineDepth: number = 200;
  ...
  private async createPipelines(): Promise<void> {
    ...
    // Uniform buffer (time, resolution, scrollX, submarineDepth)
  ...
  private render(): void {
    ...
    const bgPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.0, g: 0.35, b: 0.55, a: 1 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
  ...
  setSubmarineDepth(depth: number): void {
    this.submarineDepth = depth;
  }

  emitSubmarineTrail(x: number, y: number): void {
    this.particleSystem.emitSubmarineTrail(x, y);
  }

  emitDepthChange(x: number, y: number, direction: number): void {
    this.particleSystem.emitDepthChange(x, y, direction);
  }

  emitOxygenCollect(x: number, y: number): void {
    this.particleSystem.emitOxygenCollect(x, y);
  }

  emitStarCollect(x: number, y: number): void {
    this.particleSystem.emitStarCollect(x, y);
  }
  ...
  emitLevelStart(x: number, y: number): void {
    this.particleSystem.emitLevelStart(x, y, this.canvas.width, this.canvas.height);
  }
  ...
  emitAmbient(): void {
    this.particleSystem.emitAmbient(this.canvas.width, this.canvas.height);
  }
