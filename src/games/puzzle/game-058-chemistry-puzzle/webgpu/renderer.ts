/**
 * WebGPU Renderer - Chemistry Puzzle
...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    const renderPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.08, g: 0.10, b: 0.18, a: 1 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
  ...
  // Emit bubbles when element is added to flask
  emitDrop(x: number, y: number): void {
    this.particles.emit(x, y, 'bubble', 8);
    this.particles.emit(x, y, 'element', 5);
  }

  // Emit reaction particles
  emitReaction(x: number, y: number): void {
    this.particles.emit(x, y, 'reaction', 20);
    this.particles.emit(x, y, 'glow', 5);
    this.particles.emit(x, y, 'smoke', 8);
  }

  // Emit success glow
  emitSuccess(x: number, y: number): void {
    this.particles.emit(x, y, 'glow', 10);
    this.particles.emit(x, y, 'bubble', 12);
    this.particles.emit(x, y, 'reaction', 8);
  }

  // Emit failure smoke
  emitFail(x: number, y: number): void {
    this.particles.emit(x, y, 'smoke', 20);
    this.particles.emit(x, y, 'glow', 3);
  }

  // Emit element drag effect
  emitElement(x: number, y: number): void {
    this.particles.emit(x, y, 'element', 3);
  ...
  emitVictory(): void {
    ...
    for (let i = 0; i < 5; i++) {
      setTimeout(() => {
        ...
        this.particles.emit(x, y, 'reaction', 10);
        this.particles.emit(x, y, 'bubble', 8);
      }, i * 150);
    }
  }
  ...
  emitClear(x: number, y: number): void {
    this.particles.emit(x, y, 'bubble', 15);
    this.particles.emit(x, y, 'smoke', 5);
  }
