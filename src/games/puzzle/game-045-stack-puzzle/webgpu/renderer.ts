...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    // Update particles
    this.particles.emitAmbient(this.canvas.width);
  ...
  // Block landing with full effects
  emitLand(x: number, y: number, perfect: boolean): void {
    this.particles.emitDust(x, y, perfect ? 1.5 : 1);
    this.particles.emitImpact(x, y);
    if (perfect) {
      this.particles.emitSpark(x, y);
    }
