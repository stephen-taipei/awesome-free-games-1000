...
export class WebGPURenderer {
  ...
  // Object drag start
  emitDragStart(x: number, y: number): void {
    this.particles.emit(x, y, 'spotlight', 3);
    this.particles.emit(x, y, 'dust', 4);
  }

  // Object being dragged
  emitDragging(x: number, y: number): void {
    this.particles.emit(x, y, 'dust', 1);
  }

  // Object dropped (wrong match)
  emitDropWrong(x: number, y: number): void {
    this.particles.emit(x, y, 'shadow', 5);
  }

  // Correct match
  emitMatch(x: number, y: number): void {
    this.particles.emit(x, y, 'spark', 10);
    this.particles.emit(x, y, 'reveal', 3);
    this.particles.emit(x, y, 'spotlight', 4);
  }

  // Highlight shadow slot
  emitHighlight(x: number, y: number): void {
    this.particles.emit(x, y, 'spotlight', 2);
  }
  ...
  emitVictory(): void {
    ...
    setTimeout(() => {
      ...
      this.particles.emit(centerX, centerY, 'spotlight', 8);
    }, 900);
  }

  // Level start - ambient dust
  emitLevelStart(): void {
    for (let i = 0; i < 20; i++) {
  ...
  // Timer warning
  emitTimerWarning(): void {
    const centerX = this.canvas.width / 2;
    const centerY = this.canvas.height / 2;
    this.particles.emit(centerX, centerY, 'shadow', 3);
  }
