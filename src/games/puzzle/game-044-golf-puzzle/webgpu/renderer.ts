...
export class WebGPURenderer {
  ...
  private startAmbientParticles(): void {
    setInterval(() => {
      if (this.isInitialized && this.victoryIntensity < 0.5) {
        // Floating pollen/dust in the air
        this.particleSystem.emitAmbient(
          Math.random() * 0.8 + 0.1,
          Math.random() * 0.3 + 0.1 // Upper portion of screen (sky area)
        );
      }
    }, 300);
  }
  ...
  render(): void {
    ...
    const backgroundPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        ...
        loadOp: 'clear',
        storeOp: 'store',
        clearValue: { r: 0.2, g: 0.5, b: 0.15, a: 1 },
      }],
    });
  ...
  emitGrass(x: number, y: number): void {
    this.particleSystem.emitGrass(x, y);
  }
  ...
  emitHoleIn(x: number, y: number): void {
    this.particleSystem.emitHoleIn(x, y);
  }

  emitImpact(x: number, y: number): void {
    this.particleSystem.emitImpact(x, y);
  }
