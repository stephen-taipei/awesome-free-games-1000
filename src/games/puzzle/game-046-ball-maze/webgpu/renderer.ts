...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    // Update uniforms with tilt values
    this.device.queue.writeBuffer(
      this.bgUniformBuffer!,
      0,
      new Float32Array([time, aspect, this.tiltX, this.tiltY])
    );
    ...
    const bgPass = encoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.55, g: 0.38, b: 0.22, a: 1 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
  ...
  // Set tilt values for background effect
  setTilt(x: number, y: number): void {
    this.tiltX = x;
    this.tiltY = y;
  }

  // Emit dust from ball rolling
  emitDust(x: number, y: number, intensity: number = 1): void {
    this.particles.emitDust(x, y, intensity);
  }

  // Emit metallic sparkle
  ...
  emitTrail(x: number, y: number, vx: number, vy: number): void {
    this.particles.emitTrail(x, y, vx, vy);
  }
  ...
  emitHoleFall(x: number, y: number): void {
    this.particles.emitHoleFall(x, y);
  }
