...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    const renderPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.10, g: 0.04, b: 0.18, a: 1 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
  ...
  // Ring click effect
  emitRingClick(x: number, y: number): void {
    this.particles.emit(x, y, 'spark', 8);
    this.particles.emit(x, y, 'arcane', 5);
  }

  // Ring rotation effect
  emitRotation(x: number, y: number): void {
    this.particles.emit(x, y, 'spark', 6);
    this.particles.emit(x, y, 'glow', 3);
    this.particles.emit(x, y, 'portal', 1);
  }

  // Rune symbol activation
  emitRuneActivate(x: number, y: number): void {
    this.particles.emit(x, y, 'rune', 3);
    this.particles.emit(x, y, 'spark', 5);
  }

  // Ring alignment effect
  emitAlignment(x: number, y: number): void {
    this.particles.emit(x, y, 'arcane', 10);
    this.particles.emit(x, y, 'glow', 5);
    this.particles.emit(x, y, 'rune', 4);
  }

  // Center gem pulse
  emitGemPulse(x: number, y: number): void {
    this.particles.emit(x, y, 'glow', 8);
    this.particles.emit(x, y, 'spark', 4);
  }

  // Magic circle complete
  emitCircleComplete(x: number, y: number): void {
    this.particles.emit(x, y, 'portal', 3);
    this.particles.emit(x, y, 'arcane', 12);
    this.particles.emit(x, y, 'rune', 6);
    this.particles.emit(x, y, 'glow', 8);
  }
  ...
  emitVictory(): void {
    ...
    for (let i = 0; i < 8; i++) {
      setTimeout(() => {
        ...
        this.particles.emit(x, y, 'victory', 20);
    ...
    // Final magical burst
    setTimeout(() => {
      this.particles.emit(centerX, centerY, 'victory', 35);
      this.particles.emit(centerX, centerY, 'portal', 3);
      this.particles.emit(centerX, centerY, 'glow', 10);
      this.particles.emit(centerX, centerY, 'arcane', 15);
    }, 900);
  }
