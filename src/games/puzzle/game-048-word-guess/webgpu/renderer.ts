...
export class WebGPURenderer {
  ...
  // Emit code rain when typing
  emitCode(x: number, y: number): void {
    this.particles.emitCode(x, y);
  }

  // Emit decryption effect when letter revealed
  emitDecrypt(x: number, y: number): void {
    this.particles.emitDecrypt(x, y);
  }

  // Emit correct letter celebration
  emitCorrect(x: number, y: number): void {
    this.particles.emitCorrect(x, y);
  }
