...
export class WebGPURenderer {
  ...
  private async createPipelines(): Promise<void> {
    ...
    // Uniform buffer (time, resolution, runesAligned)
  ...
  setRunesAligned(aligned: boolean): void {
    this.runesAligned = aligned ? 1 : 0;
  }

  emitRuneRotate(x: number, y: number, runeColor: string): void {
    this.particleSystem.emitRuneRotate(x, y, runeColor);
  }

  emitRuneAligned(x: number, y: number): void {
    this.particleSystem.emitRuneAligned(x, y);
  }
