...
export class WebGPURenderer {
  ...
  // Typing a character
  emitKeyPress(x: number, y: number): void {
    this.particles.emit(x, y, 'binary', 2);
    this.particles.emit(x, y, 'circuit', 1);
  }

  // Submit answer
  emitSubmit(x: number, y: number): void {
    this.particles.emit(x, y, 'datastream', 4);
    this.particles.emit(x, y, 'circuit', 3);
  }

  // Wrong answer
  emitWrongAnswer(x: number, y: number): void {
    this.particles.emit(x, y, 'binary', 10);
    // Glitchy red particles
    for (let i = 0; i < 5; i++) {
      const offsetX = x + (Math.random() - 0.5) * 100;
      const offsetY = y + (Math.random() - 0.5) * 50;
      this.particles.emit(offsetX, offsetY, 'hexcode', 2);
    }
  }

  // Correct answer / decrypt success
  emitDecrypt(x: number, y: number): void {
    this.particles.emit(x, y, 'decrypt', 15);
    this.particles.emit(x, y, 'datastream', 6);
    this.particles.emit(x, y, 'binary', 8);
  }

  // Victory - code cracked
  emitVictory(): void {
    ...
    for (let i = 0; i < 10; i++) {
      setTimeout(() => {
        ...
        this.particles.emit(x, 0, 'matrix', 5);
      }, i * 100);
    }
    ...
    for (let i = 0; i < 8; i++) {
      setTimeout(() => {
        ...
        this.particles.emit(0, y, 'datastream', 2);
        this.particles.emit(this.canvas.width, y, 'datastream', 2);
      }, i * 80);
    }

    // Central decrypt explosion
    setTimeout(() => {
      this.particles.emit(centerX, centerY, 'decrypt', 20);
      this.particles.emit(centerX, centerY, 'binary', 15);
      this.particles.emit(centerX, centerY, 'hexcode', 10);
    }, 800);
  }

  // Level start - ambient effects
  emitLevelStart(): void {
    // Matrix drops
    for (let i = 0; i < 8; i++) {
      const x = Math.random() * this.canvas.width;
      this.particles.emit(x, 0, 'matrix', 2);
    }
    // Circuit pulses
    for (let i = 0; i < 5; i++) {
      const x = Math.random() * this.canvas.width;
  ...
  emitReset(): void {
    const centerX = this.canvas.width / 2;
    const centerY = this.canvas.height / 2;
    this.particles.emit(centerX, centerY, 'datastream', 4);
    this.particles.emit(centerX, centerY, 'binary', 6);
  }
