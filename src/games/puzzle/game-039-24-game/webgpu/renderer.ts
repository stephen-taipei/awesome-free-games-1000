/**
 * WebGPU Renderer - 24 Game
 * Mental Math Arena / Calculator Championship Theme
 * Game #039
 */

import { backgroundShader, particleShader, targetShader, victoryShader } from './shaders';
...
export class WebGPURenderer {
  ...
  private targetUniformBuffer!: GPUBuffer;
  private targetBindGroup!: GPUBindGroup;
  ...
  private setupTargetPipeline(): void {
    const shaderModule = this.device.createShaderModule({
      code: targetShader,
    });

    this.targetUniformBuffer = this.device.createBuffer({
      size: 16,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    ...
    this.targetBindGroup = this.device.createBindGroup({
      layout: bindGroupLayout,
      entries: [
        { binding: 0, resource: { buffer: this.targetUniformBuffer } },
      ],
    });

    this.targetPipeline = this.device.createRenderPipeline({
      layout: this.device.createPipelineLayout({ bindGroupLayouts: [bindGroupLayout] }),
      vertex: {
        module: shaderModule,
        entryPoint: 'vertexMain',
  ...
  render(): void {
    ...
    this.device.queue.writeBuffer(
      this.targetUniformBuffer,
      0,
      new Float32Array([time, this.targetIntensity, 0, 0])
    );
    ...
    // Target highlight pass
    if (this.targetIntensity > 0.01) {
      const targetPass = commandEncoder.beginRenderPass({
        colorAttachments: [{
          view: textureView,
          loadOp: 'load',
          storeOp: 'store',
      ...
      targetPass.setBindGroup(0, this.targetBindGroup);
      targetPass.draw(6);
      targetPass.end();
    }
  ...
  emitOperator(x: number, y: number): void {
    this.particleSystem.emitOperator(x, y);
  }

  emitCalculate(x: number, y: number): void {
    this.particleSystem.emitCalculate(x, y);
  }
  ...
  emitError(x: number, y: number): void {
    this.particleSystem.emitError(x, y);
  }
  ...
  destroy(): void {
    ...
    this.targetUniformBuffer?.destroy();
