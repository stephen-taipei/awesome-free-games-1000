...
export interface GateData {
  ...
  y: number;
  active: boolean;
  gateType: number; // 0=INPUT, 1=AND, 2=OR, 3=NOT, 4=OUTPUT
}
...
export class WebGPURenderer {
  ...
  emitSignalFlow(fromX: number, fromY: number, toX: number, toY: number): void {
    this.particleSystem.emitSignalFlow(fromX, fromY, toX, toY);
  }

  emitGateActivation(x: number, y: number, isHigh: boolean): void {
    this.particleSystem.emitGateActivation(x, y, isHigh);
  }
  ...
  render(deltaTime: number): void {
    ...
    // Decay victory
    this.victoryProgress *= 0.98;
    if (this.victoryProgress < 0.01) this.victoryProgress = 0;
    ...
    const renderPass = encoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.01, g: 0.04, b: 0.06, a: 1.0 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
