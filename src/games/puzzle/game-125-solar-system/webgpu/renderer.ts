...
export class WebGPURenderer {
  ...
  private sunX: number = 250;
  private sunY: number = 200;
  ...
  private async createPipelines(): Promise<void> {
    ...
    // Uniform buffer (time, resolution, sunX, sunY)
  ...
  setSunPosition(x: number, y: number): void {
    this.sunX = x;
    this.sunY = y;
  }

  emitPlanetClick(x: number, y: number, color: [number, number, number]): void {
    this.particleSystem.emitPlanetClick(x, y, color);
  }

  emitAlignment(x: number, y: number): void {
    this.particleSystem.emitAlignment(x, y);
  }

  emitSolarFlare(): void {
    this.particleSystem.emitSolarFlare(this.sunX, this.sunY);
  }
