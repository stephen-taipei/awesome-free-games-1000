/**
 * WebGPU Renderer - Yin Yang Balance
...
export class WebGPURenderer {
  ...
  private setupBuffers(): void {
    this.uniformBuffer = this.device.createBuffer({
      size: 16, // time, width, height, balance
  ...
  render(): void {
    ...
    this.device.queue.writeBuffer(
      this.uniformBuffer,
      0,
      new Float32Array([time, this.canvas.width, this.canvas.height, this.currentBalance])
    );
  ...
  // Element placed
  emitElementPlace(x: number, y: number, isYin: boolean): void {
    this.particles.emit(x, y, 'energy', 6, isYin);
    this.particles.emit(x, y, 'flow', 4, isYin);
    this.particles.emit(x, y, 'spark', 8, isYin);
  }

  // Balance achieved
  emitBalanceAchieved(x: number, y: number): void {
    this.particles.emit(x, y, 'harmony', 5);
    this.particles.emit(x, y, 'balance', 8);
    this.particles.emit(x, y, 'wave', 3);
  }

  // Imbalance warning
  emitImbalance(x: number, y: number): void {
    this.particles.emit(x, y, 'wave', 2, false);
  }

  // Type selected
  emitTypeSelect(x: number, y: number, isYin: boolean): void {
    this.particles.emit(x, y, 'energy', 3, isYin);
  }
  ...
  emitVictory(): void {
    ...
    for (let i = 0; i < 4; i++) {
      setTimeout(() => {
        ...
        this.particles.emit(x, y, 'energy', 5, i % 2 === 0);
        this.particles.emit(x, y, 'spark', 10, i % 2 === 0);
      }, i * 150);
    }

    // Central harmony burst
    setTimeout(() => {
      this.particles.emit(centerX, centerY, 'harmony', 6);
      this.particles.emit(centerX, centerY, 'balance', 10);
    }, 600);

    // Expanding waves
    setTimeout(() => {
      this.particles.emit(centerX, centerY, 'wave', 5, true);
      this.particles.emit(centerX, centerY, 'wave', 5, false);
    }, 800);
  }
  ...
  emitLevelStart(): void {
    ...
    this.particles.emit(centerX - 50, centerY, 'energy', 4, true);
    this.particles.emit(centerX + 50, centerY, 'energy', 4, false);
    this.particles.emit(centerX, centerY, 'balance', 5);
  }
  ...
  emitReset(): void {
    const centerX = this.canvas.width / 2;
    const centerY = this.canvas.height / 2;
    this.particles.emit(centerX, centerY, 'wave', 4, true);
    this.particles.emit(centerX, centerY, 'wave', 4, false);
  }
  ...
  emitGameComplete(): void {
    ...
    // Grand duality finale
    for (let wave = 0; wave < 5; wave++) {
      setTimeout(() => {
        for (let i = 0; i < 8; i++) {
          const angle = (i / 8) * Math.PI * 2 + wave * 0.2;
          const dist = 60 + wave * 25;
          const x = centerX + Math.cos(angle) * dist;
          const y = centerY + Math.sin(angle) * dist;
          this.particles.emit(x, y, 'spark', 6, i % 2 === 0);
          this.particles.emit(x, y, 'energy', 3, i % 2 === 0);
        }
      }, wave * 180);
    }

    // Central perfect harmony
    setTimeout(() => {
      this.particles.emit(centerX, centerY, 'harmony', 8);
      this.particles.emit(centerX, centerY, 'balance', 12);
      this.particles.emit(centerX, centerY, 'wave', 6, true);
      this.particles.emit(centerX, centerY, 'wave', 6, false);
    }, 900);
  }
