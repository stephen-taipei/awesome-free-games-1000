...
export class WebGPURenderer {
  ...
  render(): void {
    ...
    const backgroundPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        ...
        loadOp: 'clear',
        storeOp: 'store',
        clearValue: { r: 0.98, g: 0.96, b: 0.92, a: 1 },
      }],
    });
  ...
  emitSplash(x: number, y: number, color: string): void {
    this.particleSystem.emitSplash(x, y, color);
  }

  emitSpread(x: number, y: number, color: string): void {
    this.particleSystem.emitSpread(x, y, color);
  }

  emitRipple(x: number, y: number, color: string): void {
    this.particleSystem.emitRipple(x, y, color);
  }
