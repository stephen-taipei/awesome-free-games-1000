/**
 * WebGPU Renderer - Rainbow Bridge
 * Rainbow / Sky Theme
 * Game #129
 */
...
export class WebGPURenderer {
  ...
  private playerX: number = 0.5;
  private playerY: number = 0.5;
  ...
  emitColorSelect(x: number, y: number, color: number[]) {
    this.particleSystem.emitColorSelect(x, y, color);
  }

  // Color placed
  emitColorPlace(x: number, y: number, color: number[]) {
    this.particleSystem.emitColorPlace(x, y, color);
  }

  // Wrong placement
  emitWrongPlace(x: number, y: number) {
    this.particleSystem.emitWrongPlace(x, y);
  }

  // Bridge complete
  emitBridgeComplete(startX: number, endX: number, y: number) {
    this.particleSystem.emitBridgeComplete(startX, endX, y);
  }
