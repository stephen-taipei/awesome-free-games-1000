/**
 * WebGPU Renderer - Castle Mechanism
...
export class WebGPURenderer {
  ...
  private async createPipelines(): Promise<void> {
    ...
    // Uniform buffer (time, resolution, gateProgress)
  ...
  private render(): void {
    ...
    const bgPass = commandEncoder.beginRenderPass({
      colorAttachments: [{
        view: textureView,
        clearValue: { r: 0.25, g: 0.24, b: 0.22, a: 1 },
        loadOp: 'clear',
        storeOp: 'store',
      }],
    });
  ...
  setGateProgress(progress: number): void {
    this.gateProgress = progress;
  }

  emitMechanismActivate(x: number, y: number, mechType: string): void {
    this.particleSystem.emitMechanismActivate(x, y, mechType);
  }

  emitChainReaction(x: number, y: number): void {
    this.particleSystem.emitChainReaction(x, y);
  }

  emitWrongOrder(): void {
    this.particleSystem.emitWrongOrder(this.canvas.width, this.canvas.height);
  }

  emitGateProgress(x: number, y: number): void {
    this.particleSystem.emitGateProgress(x, y);
  }
  ...
  emitLevelStart(): void {
    this.particleSystem.emitLevelStart(this.canvas.width, this.canvas.height);
  }
