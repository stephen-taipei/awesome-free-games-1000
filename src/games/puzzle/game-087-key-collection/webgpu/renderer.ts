/**
 * WebGPU Renderer - Key Collection
...
export class WebGPURenderer {
  ...
  private renderFrame(): void {
    ...
    if (Math.random() < 0.02) {
      ...
      this.particleSystem.emit(x, y, 'dust', 1, Math.floor(Math.random() * 6));
    }
  ...
  private getParticleTypeIndex(type: ParticleType): number {
    const types: ParticleType[] = ['glow', 'spark', 'trail', 'shimmer', 'burst', 'dust'];
    return types.indexOf(type);
  }

  // Effect methods
  emitMove(x: number, y: number): void {
    this.particleSystem.emit(x, y, 'dust', 2, 0);
  }

  emitKeyCollect(x: number, y: number): void {
    this.activity = 1;
    this.particleSystem.emit(x, y, 'shimmer', 8, 0);
    this.particleSystem.emit(x, y, 'spark', 10, 0);
    this.particleSystem.emit(x, y, 'glow', 3, 0);
  }

  emitDoorUnlock(x: number, y: number): void {
    this.activity = 1;
    this.particleSystem.emit(x, y, 'burst', 8, 0);
    this.particleSystem.emit(x, y, 'shimmer', 10, 0);
    this.particleSystem.emit(x, y, 'glow', 4, 0);
  }

  emitDoorBlocked(x: number, y: number): void {
    this.activity = 0.5;
    for (let i = 0; i < 4; i++) {
      const angle = (i / 4) * Math.PI * 2;
      const px = x + Math.cos(angle) * 20;
      const py = y + Math.sin(angle) * 20;
      this.particleSystem.emit(px, py, 'spark', 1, 3);
    }
  }
    ...
    for (let wave = 0; wave < 4; wave++) {
      setTimeout(() => {
        for (let i = 0; i < 16; i++) {
          ...
          this.particleSystem.emit(px, py, 'shimmer', 2, 0);
        }
  ...
  emitLevelStart(x: number, y: number): void {
    ...
    // Dungeon entrance effect
    for (let i = 0; i < 15; i++) {
      const px = Math.random() * this.canvas.width;
      const py = Math.random() * this.canvas.height;
      this.particleSystem.emit(px, py, 'dust', 1, i % 6);
    }
    this.particleSystem.emit(x, y, 'glow', 3, 0);
    this.particleSystem.emit(x, y, 'shimmer', 5, 0);
  }
  ...
  emitReset(): void {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      this.particleSystem.emit(px, py, 'dust', 3, i % 6);
    }
    this.activity = 0;
  }
