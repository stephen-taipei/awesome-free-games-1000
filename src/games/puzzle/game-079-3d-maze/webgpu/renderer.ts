...
import { neonPulse, depthFog } from './math';
...
export class WebGPURenderer {
  ...
  private fogIntensity: number = 0.5;
  ...
  private renderFrame(): void {
    ...
    // Update uniforms
    const uniformData = new Float32Array([time, width, height, this.playerAngle]);
    this.device.queue.writeBuffer(this.uniformBuffer, 0, uniformData);

    // Update particles
    this.particleSystem.update(1 / 60);

    // Emit ambient fog particles
    if (Math.random() < 0.1) {
      const x = Math.random() * width;
      const y = Math.random() * height;
      this.particleSystem.emit(x, y, 'fog', 1);
    }
  ...
  private getParticleTypeIndex(type: ParticleType): number {
    const types: ParticleType[] = ['fog', 'portal', 'spark', 'dust', 'glow', 'trail'];
    return types.indexOf(type);
  }

  // Effect methods for game events
  emitMove(x: number, y: number): void {
    this.particleSystem.emit(x, y, 'dust', 3);
    this.particleSystem.emit(x, y, 'trail', 2);
  }

  emitTurn(x: number, y: number, direction: number): void {
    this.playerAngle = direction;
    this.particleSystem.emit(x, y, 'glow', 4, Math.floor(direction / 90) % 5);
  }

  emitWallProximity(x: number, y: number): void {
    this.particleSystem.emit(x, y, 'spark', 2, 0);
    this.particleSystem.emit(x, y, 'dust', 1);
  }

  emitPortal(x: number, y: number): void {
    this.particleSystem.emit(x, y, 'portal', 5);
  ...
  emitGoalReached(x: number, y: number): void {
    for (let i = 0; i < 8; i++) {
      ...
      this.particleSystem.emit(px, py, 'portal', 3);
      this.particleSystem.emit(px, py, 'spark', 2, i % 5);
    }
    this.particleSystem.emit(x, y, 'glow', 10, 2);
  }
  ...
  emitLevelStart(x: number, y: number): void {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      this.particleSystem.emit(px, py, 'fog', 2);
    }
    this.particleSystem.emit(x, y, 'glow', 5, 0);
    this.particleSystem.emit(x, y, 'portal', 3);
  }
  ...
  emitReset(): void {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      this.particleSystem.emit(px, py, 'dust', 3);
    }
    this.playerAngle = 0;
  }
