...
export class KnightDashGame {
  ...
  private updatePlayer(dt: number): void {
    ...
    player.frame = (player.frame + dt * 0.018) % 4;
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: ("goblin" | "orc" | "spike" | "dragon")[] = ["goblin", "orc", "spike", "dragon"];
      const type = types[Math.floor(Math.random() * types.length)];

      let width = 32, height = 36;
      if (type === "orc") { width = 40; height = 44; }
      if (type === "spike") { width = 36; height = 28; }
      if (type === "dragon") { width = 44; height = 32; }

      this.state.obstacles.push({
        x: ARENA_WIDTH + 50,
        y: type === "dragon" ? GROUND_Y - 50 : GROUND_Y - height / 2,
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 1000) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: ("gem" | "sword" | "holy")[] = ["gem", "gem", "gem", "sword", "holy"];
      const type = types[Math.floor(Math.random() * types.length)];
  ...
  private updateShieldTime(dt: number): void {
    if (this.state.hasShield) {
  ...
  private spawnShieldBurst(x: number, y: number): void {
    const colors = ["#ffd700", "#4169e1", "#ffffff", "#c0c0c0"];
    for (let i = 0; i < 16; i++) {
  ...
  public handleKeyDown(code: string): void {
    switch (code) {
      ...
      case "ArrowDown": case "KeyS": this.shieldBash(); break;
    }
  }
