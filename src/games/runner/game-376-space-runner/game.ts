...
export interface PendingEvents {
  ...
  collision: boolean;
  gameOver: { score: number; distance: number; stars: number }[];
}
...
export interface Obstacle {
  ...
  width: number;
  height: number;
  type: 'meteor' | 'satellite' | 'laser' | 'debris';
  lane: number;
}
...
export class SpaceRunnerGame {
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: ('meteor' | 'satellite' | 'laser' | 'debris')[] = ['meteor', 'satellite', 'laser', 'debris'];
      const type = types[Math.floor(Math.random() * types.length)];

      let width = 40, height = 40;
      if (type === 'laser') { width = 60; height = 20; }
      if (type === 'satellite') { width = 50; height = 50; }

      this.state.obstacles.push({
        x: ARENA_WIDTH + 50,
        y: type === 'laser' ? GROUND_Y - 30 : GROUND_Y - height / 2,
        width,
        height,
        type,
        lane,
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 1200) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: ('star' | 'energy' | 'shield')[] = ['star', 'star', 'star', 'energy', 'shield'];
      const type = types[Math.floor(Math.random() * types.length)];
