...
export interface Obstacle {
  ...
  width: number;
  height: number;
  type: "asteroid" | "comet" | "blackhole" | "satellite";
  lane: number;
  rotation: number;
}
...
export class StarRunGame {
  ...
  private updatePlayer(dt: number): void {
    ...
    // Star trail
    if (Math.random() < 0.4) {
      const colors = ["#ff66ff", "#66ffff", "#ffff66", "#ffffff"];
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: ("asteroid" | "comet" | "blackhole" | "satellite")[] = [
        "asteroid",
        "comet",
        "blackhole",
        "satellite",
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 1100) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: ("stardust" | "nebula" | "warp")[] = [
        "stardust",
        "stardust",
        "stardust",
        "nebula",
