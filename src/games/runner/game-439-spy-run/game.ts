...
export interface Obstacle {
  ...
  width: number;
  height: number;
  type: 'laser-grid' | 'turret' | 'patrol' | 'sensor' | 'door' | 'drone';
  lane: number;
}
...
export class SpyRunGame {
  ...
  private updateDetection(dt: number): void {
    // Detection decreases over time
    this.state.detected = Math.max(0, this.state.detected - dt * 0.008);
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: Obstacle['type'][] = ['laser-grid', 'turret', 'patrol', 'sensor', 'door', 'drone'];
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 850) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: Collectible['type'][] = ['intel', 'intel', 'gadget', 'key'];
      const type = types[Math.floor(Math.random() * types.length)];
  ...
  private checkCollisions(): void {
    ...
    for (const obs of obstacles) {
      ...
      if (playerBox.right > obsBox.left && playerBox.left < obsBox.right &&
        ...
        if ((obs.type === 'turret' || obs.type === 'patrol' || obs.type === 'door') && player.isJumping) {
          this.state.score += 60;
          this.spawnStyleParticles(obs.x, obs.y, '#00d4aa');
          obs.x = -200;
          continue;
        }

        if ((obs.type === 'laser-grid' || obs.type === 'sensor' || obs.type === 'drone') && player.isSliding) {
          this.state.score += 80;
