...
export class SnowSlideGame {
  ...
  private updateObstacles(dt: number): void {
    const speed = this.state.speed * (this.state.hasBoost ? 1.5 : 1) * (dt / 16);
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: Obstacle['type'][] = ['tree', 'rock', 'snowman', 'log'];
      const type = types[Math.floor(Math.random() * types.length)];

      let width = 40, height = 60;
      if (type === 'rock') { width = 35; height = 30; }
      if (type === 'log') { width = 50; height = 25; }
      if (type === 'snowman') { width = 35; height = 50; }
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 1000) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: Collectible['type'][] = ['coin', 'coin', 'coin', 'gem', 'boost'];
      const type = types[Math.floor(Math.random() * types.length)];
  ...
  private updateScore(dt: number): void {
    this.state.distance += this.state.speed * (this.state.hasBoost ? 1.5 : 1) * (dt / 16);
