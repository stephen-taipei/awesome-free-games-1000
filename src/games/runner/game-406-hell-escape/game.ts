...
export interface Obstacle {
  ...
  width: number;
  height: number;
  type: "fireball" | "demon" | "spike" | "lava";
  lane: number;
}
...
export class HellEscapeGame {
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: ("fireball" | "demon" | "spike" | "lava")[] = [
        "fireball",
        "demon",
        "spike",
        "lava",
      ...
      this.state.obstacles.push({
        x: ARENA_WIDTH + 50,
        y: type === "lava" ? GROUND_Y + 10 : GROUND_Y - height / 2,
        width,
        height,
        type,
        lane,
