/**
 * Ocean Run Game Logic
 * Game #407 - Ocean Runner
 */
...
export interface PendingEvents {
  ...
  collision: boolean;
  gameOver: { score: number; distance: number; pearls: number }[];
}
...
export interface Obstacle {
  ...
  width: number;
  height: number;
  type: "jellyfish" | "shark" | "urchin" | "kelp";
  lane: number;
}
...
export class OceanRunGame {
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: ("jellyfish" | "shark" | "urchin" | "kelp")[] = [
        "jellyfish",
        "shark",
        "urchin",
        "kelp",
      ...
      this.state.obstacles.push({
        x: ARENA_WIDTH + 50,
        y: type === "kelp" ? GROUND_Y - 20 : GROUND_Y - height / 2,
        width,
        height,
        type,
        lane,
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 1100) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: ("pearl" | "shell" | "bubble")[] = [
        "pearl",
        "pearl",
        "pearl",
        "shell",
    ...
    for (const obs of obstacles) {
      ...
      if (
        ...
        if (this.state.hasShield) {
          ...
          this.spawnBubblePop(obs.x, obs.y);
          obs.x = -100;
        } else {
  ...
  private gameOver(): void {
    ...
    this.stopGameLoop();
    this.spawnBubblePop(this.state.player.x, this.state.player.y);
  }
  ...
  public jump(): void {
    ...
    if (!this.state.player.isJumping) {
      ...
      this.state.player.jumpVelocity = -12;
