...
startBtn.addEventListener('click', () => {
  ...
  if (!game) {
    ...
    game.onScore = (s: number) => { scoreDisplay.textContent = String(s); };
    game.onLevel = (l: number) => { levelDisplay.textContent = String(l); };
    game.onGameOver = () => { overlay.style.display = 'flex'; document.getElementById('overlay-title')!.textContent = 'Game Over!'; };
  }
  game.start();
});
applyTranslations();
