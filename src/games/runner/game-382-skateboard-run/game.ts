...
export class SkateboardRunGame {
  state: GameState;
  ...
  private updatePlayer(dt: number): void {
    ...
    // Trick rotation
    if (player.isTricking && player.isJumping) {
      player.trickRotation += 0.3 * (dt / 16);
    }
  ...
  private spawnObstacles(dt: number): void {
    ...
    if (this.spawnTimer >= spawnInterval) {
      ...
      const types: ('cone' | 'bench' | 'rail' | 'car')[] = ['cone', 'bench', 'rail', 'car'];
      const type = types[Math.floor(Math.random() * types.length)];

      let width = 35, height = 35;
      if (type === 'bench') { width = 60; height = 30; }
      if (type === 'rail') { width = 50; height = 20; }
      if (type === 'car') { width = 70; height = 45; }

      this.state.obstacles.push({
        x: ARENA_WIDTH + 50,
        y: type === 'rail' ? GROUND_Y - 30 : GROUND_Y - height / 2,
        width,
        height,
        type,
        lane,
  ...
  private spawnCollectibles(dt: number): void {
    ...
    if (this.collectibleTimer >= 1000) {
      this.collectibleTimer = 0;
      const lane = Math.floor(Math.random() * 3);
      const types: ('coin' | 'gear' | 'spray')[] = ['coin', 'coin', 'coin', 'gear', 'spray'];
      const type = types[Math.floor(Math.random() * types.length)];
  ...
  private spawnExplosion(x: number, y: number): void {
    for (let i = 0; i < 20; i++) {
      ...
      this.state.particles.push({
        ...
        life: 30,
        maxLife: 30,
        color: ['#e74c3c', '#f39c12', '#95a5a6'][Math.floor(Math.random() * 3)],
  ...
  public trick(): void {
    if (this.state.phase !== 'playing') return;
    if (this.state.player.isJumping && !this.state.player.isTricking) {
