...
export class WebGPURenderer {
  ...
  // Emission methods
  emitBombPlace(x: number, y: number) {
    this.particleSystem.emit('bombPlace', x, y);
  }

  emitExplosion(x: number, y: number, power: number = 1) {
    this.particleSystem.emit('explosion', x, y, { power });
  }

  emitBrickDestroy(x: number, y: number) {
    this.particleSystem.emit('brickDestroy', x, y);
  }
  ...
  emitPowerUp(x: number, y: number, powerUpType: 'bomb' | 'power' | 'speed' = 'bomb') {
    this.particleSystem.emit('powerUp', x, y, { powerUpType });
  }
