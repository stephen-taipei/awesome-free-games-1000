...
function initGame() {
  game = new IceFireGame(canvas);
  game.resize();

  game.setOnStateChange((state: any) => {
    if (state.gems !== undefined) {
...
function showWin() {
  setTimeout(() => {
    ...
    overlayMsg.textContent = `${i18n.t("game.gems")}: ${game.getGems()}`;
