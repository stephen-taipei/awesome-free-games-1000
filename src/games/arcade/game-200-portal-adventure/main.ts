/**
 * Portal Adventure Main Entry
 * Game #200
 */
import { PortalAdventureGame } from "./game";
...
function initGame() {
  game = new PortalAdventureGame(canvas);
  game.resize();

  game.setOnStateChange((state: any) => {
    if (state.keys !== undefined) {
      keysDisplay.textContent = `${state.keys}/${state.totalKeys}`;
    }
