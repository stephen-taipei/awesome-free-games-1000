/**
 * WebGPU Renderer - Space Invaders
...
export class WebGPURenderer {
  ...
  emitPlayerShoot(x: number, y: number) {
    this.particleSystem.emitPlayerShoot(x, y);
  }

  emitAlienDeath(x: number, y: number, alienType: number) {
    this.particleSystem.emitAlienDeath(x, y, alienType);
  }

  emitPlayerHit(x: number, y: number) {
    this.particleSystem.emitPlayerHit(x, y);
  }

  emitBarrierHit(x: number, y: number) {
    this.particleSystem.emitBarrierHit(x, y);
  }

  emitBulletTrail(x: number, y: number, isPlayer: boolean) {
    this.particleSystem.emitBulletTrail(x, y, isPlayer);
  }
  ...
  emitLevelComplete() {
    this.particleSystem.emitLevelComplete();
  }
