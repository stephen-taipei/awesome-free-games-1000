/**
 * Airport Control Main Entry
 * Game #252
 */
import { AirportControlGame } from "./game";
...
function initGame() {
  game = new AirportControlGame(canvas);
  game.resize();
  ...
  game.setOnStateChange((state) => {
    ...
    landedDisplay.textContent = state.landed.toString();

    if (state.status === "over") {
      showGameOver(state.score, state.landed);
    }
  });
...
function showGameOver(score: number, landed: number) {
  setTimeout(() => {
    overlay.style.display = "flex";
    overlayTitle.textContent = i18n.t("game.over");
    overlayMsg.textContent = `${i18n.t("game.score")}: ${score} | ${i18n.t("game.landed")}: ${landed}`;
