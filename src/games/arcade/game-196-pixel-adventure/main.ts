/**
 * Pixel Adventure Main Entry
 * Game #196
 */
import { PixelAdventureGame } from "./game";
...
function initGame() {
  game = new PixelAdventureGame(canvas);
  game.resize();

  game.setOnStateChange((state: any) => {
    if (state.coins !== undefined) {
      coinsDisplay.textContent = `${state.coins}/${state.totalCoins}`;
    }
...
function startGame() {
  ...
  coinsDisplay.textContent = `${game.getCoins()}/0`;
  livesDisplay.textContent = game.getLives().toString();
}
