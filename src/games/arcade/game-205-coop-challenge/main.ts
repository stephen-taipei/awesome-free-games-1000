/**
 * Co-op Challenge Main Entry
 * Game #205
 */
import { CoopChallengeGame } from "./game";
...
function initGame() {
  game = new CoopChallengeGame(canvas);
  game.resize();
...
function showWin() {
  setTimeout(() => {
    ...
    overlayMsg.textContent = `${i18n.t("game.time")}: ${game.getTime()}s`;
...
function startGame() {
  ...
  timeDisplay.textContent = game.getTime().toString();
}
