/**
 * WebGPU Renderer - Basketball Arcade
...
import { BASKETBALL_COLORS, randomRange, getRandomConfettiColor } from './math';
...
export class WebGPURenderer {
  ...
  // Emission methods
  emitShoot(x: number, y: number, vx: number, vy: number) {
    // Trail behind the ball
    for (let i = 0; i < 5; i++) {
      this.particleSystem.emit(x, y, 'shoot', 1, {
        direction: [-vx, -vy],
        spread: 0.3,
        speed: randomRange(0.01, 0.03),
      });
    }
  }
  ...
  emitScore(x: number, y: number, points: number) {
    // Score burst
    const count = points === 3 ? 25 : 15;
    this.particleSystem.emit(x, y, 'score', count, {
      spread: Math.PI * 2,
      speed: randomRange(0.08, 0.15),
    });

    // Extra sparkles for 3-pointers
    if (points === 3) {
      this.particleSystem.emit(x, y, 'swish', 15, {
        spread: Math.PI * 2,
  ...
  emitRimHit(x: number, y: number) {
    this.particleSystem.emit(x, y, 'rimHit', 8, {
      spread: Math.PI,
      direction: [0, -1],
  ...
  emitSwish(x: number, y: number) {
    // Clean swish - net sparkles
    this.particleSystem.emit(x, y, 'swish', 20, {
      spread: Math.PI * 0.5,
      direction: [0, 1],
      speed: randomRange(0.01, 0.03),
    });
    ...
    for (let i = 0; i < 10; i++) {
      this.particleSystem.emit(
        ...
        1,
        {
          ...
          color: BASKETBALL_COLORS.scoreGold,
        }
      );
    }
  }
  ...
  emitGameOver(x: number, y: number, victory: boolean) {
    if (victory) {
      ...
      this.particleSystem.emit(0.5, 0.5, 'score', 30, {
        color: BASKETBALL_COLORS.scoreGold,
        spread: Math.PI * 2,
        speed: randomRange(0.1, 0.2),
      });
    } else {
      // Game over effect
      this.particleSystem.emit(x, y, 'miss', 20, {
        spread: Math.PI * 2,
