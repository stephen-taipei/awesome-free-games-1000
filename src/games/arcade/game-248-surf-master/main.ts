/**
 * Surf Master Main Entry
 * Game #248
 */
import { SurfMasterGame } from "./game";
...
function initGame() {
  game = new SurfMasterGame(canvas);
  game.resize();
...
function showGameOver(score: number, distance: number) {
  setTimeout(() => {
    ...
    overlayMsg.textContent = `${i18n.t("game.score")}: ${score} | ${i18n.t("game.distance")}: ${distance}m`;
