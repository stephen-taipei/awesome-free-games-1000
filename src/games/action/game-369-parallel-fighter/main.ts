import { ParallelFighterGame } from './game';
...
function initGame() {
  game = new ParallelFighterGame(canvas);
  game.resize();
  game.setOnStateChange((state) => {
    scoreDisplay.textContent = state.score.toString();
    highDisplay.textContent = state.highScore.toString();
    dimensionDisplay.textContent = state.activeDimension === 0 ? 'A' : 'B';
    dimensionDisplay.className = `stat-value dim-${state.activeDimension === 0 ? 'a' : 'b'}`;
...
['left', 'right', 'jump', 'attack', 'switch'].forEach(action => {
