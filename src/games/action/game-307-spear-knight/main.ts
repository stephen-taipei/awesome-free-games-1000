import { SpearKnightGame } from './game';
...
function initGame() {
  game = new SpearKnightGame(canvas);
  game.resize();
  game.setOnStateChange((state) => {
    ...
    dashFill.style.width = `${Math.max(0, (60 - state.dashCooldown) / 60) * 100}%`;
...
['left', 'right', 'up', 'down', 'attack', 'dash'].forEach(action => {
