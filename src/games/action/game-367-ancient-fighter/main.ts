import { AncientFighterGame } from './game';
...
function initGame() {
  game = new AncientFighterGame(canvas);
  game.resize();
  game.setOnStateChange((state) => {
    scoreDisplay.textContent = Math.floor(state.score).toString();
    highDisplay.textContent = Math.floor(state.highScore).toString();
...
['left', 'right', 'jump', 'sword', 'shield', 'charge'].forEach(action => {
