/**
 * Transcender Main Entry
 * Game #362
 */
import { TranscenderGame } from "./game";
import { translations } from "./i18n";
import { i18n, type Locale } from "../../../shared/i18n";

const canvas = document.getElementById("game-canvas") as HTMLCanvasElement;
const languageSelect = document.getElementById("language-select") as HTMLSelectElement;
const transcendDisplay = document.getElementById("transcend-display")!;
...
function initGame() {
  game = new TranscenderGame(canvas);
  game.resize();
  ...
  btnShift.addEventListener("click", () => game.shift());
  ...
  game.setOnStateChange((state) => {
    transcendDisplay.textContent = `${state.transcend}%`;
    dimensionDisplay.textContent = state.dimension.toString();
