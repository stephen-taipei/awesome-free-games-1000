/**
 * Angel Warrior Game Engine
 * Game #337
 *
 * Angelic warrior with divine powers and celestial wings!
 */

interface Player {
  x: number;
  y: number;
  width: number;
  height: number;
  vx: number;
  vy: number;
  health: number;
  maxHealth: number;
  divine: number;
  maxDivine: number;
  facing: "left" | "right";
  isJumping: boolean;
  isAttacking: boolean;
  attackTimer: number;
  invincible: boolean;
  invincibleTimer: number;
  angelForm: boolean;
  angelFormTimer: number;
  canFly: boolean;
}

interface Enemy {
  x: number;
  y: number;
  width: number;
  height: number;
  vx: number;
  type: "shadow" | "dark-knight" | "void-lord";
  health: number;
  maxHealth: number;
  facing: "left" | "right";
  attackTimer: number;
  patrolLeft: number;
  patrolRight: number;
}

interface Platform {
  x: number;
  y: number;
  width: number;
  height: number;
}

interface LightBeam {
  x: number;
  y: number;
  vx: number;
  vy: number;
  size: number;
  life: number;
}

interface Feather {
  x: number;
  y: number;
  vx: number;
  vy: number;
  life: number;
}

interface GameState {
  score: number;
  health: number;
  divine: number;
  level: number;
  status: "idle" | "playing" | "clear" | "over";
}

type StateCallback = (state: GameState) => void;

const GRAVITY = 0.6;
const JUMP_FORCE = -14;
const MOVE_SPEED = 5;
const ANGEL_SPEED = 7;
const FLIGHT_SPEED = 0.3;
const ATTACK_DURATION = 20;

export class AngelWarriorGame {
  private canvas: HTMLCanvasElement;
  private ctx: CanvasRenderingContext2D;
  private player: Player;
  private enemies: Enemy[] = [];
  private platforms: Platform[] = [];
  private lightBeams: LightBeam[] = [];
  private feathers: Feather[] = [];
  private score = 0;
  private level = 1;
  private status: GameState["status"] = "idle";
  private onStateChange: StateCallback | null = null;
  private animationId: number | null = null;
  private keys = { left: false, right: false, jump: false, attack: false, special: false };
  private cameraX = 0;
  private levelWidth = 0;
  private divineRegenTimer = 0;

  constructor(canvas: HTMLCanvasElement) {
    this.canvas = canvas;
    this.ctx = canvas.getContext("2d")!;
    this.player = this.createPlayer();
  }

  private createPlayer(): Player {
    return {
      x: 50,
      y: 200,
      width: 32,
      height: 46,
      vx: 0,
      vy: 0,
      health: 100,
      maxHealth: 100,
      divine: 50,
      maxDivine: 100,
      facing: "right",
      isJumping: false,
      isAttacking: false,
      attackTimer: 0,
      invincible: false,
      invincibleTimer: 0,
      angelForm: false,
      angelFormTimer: 0,
      canFly: false,
    };
  }

  setOnStateChange(cb: StateCallback) {
    this.onStateChange = cb;
  }

  private emitState() {
    if (this.onStateChange) {
      this.onStateChange({
        score: this.score,
        health: this.player.health,
        divine: Math.floor(this.player.divine),
        level: this.level,
        status: this.status,
      });
    }
  }

  resize() {
    const rect = this.canvas.parentElement!.getBoundingClientRect();
    this.canvas.width = rect.width;
    this.canvas.height = rect.height;
    this.draw();
  }

  start() {
    this.score = 0;
    this.level = 1;
    this.player = this.createPlayer();
    this.setupLevel();
    this.status = "playing";
    this.emitState();
    this.gameLoop();
  }

  private setupLevel() {
    this.enemies = [];
    this.platforms = [];
    this.lightBeams = [];
    this.feathers = [];
    this.player.x = 50;
    this.player.y = 200;
    this.player.health = this.player.maxHealth;
    this.player.divine = 50;
    this.cameraX = 0;

    const h = this.canvas.height;
    this.levelWidth = this.canvas.width * 3;

    // Ground
    this.platforms.push({
      x: 0,
      y: h - 40,
      width: this.levelWidth,
      height: 40,
    });

    // Celestial platforms
    const platformCount = 6 + this.level * 2;
    for (let i = 0; i < platformCount; i++) {
      this.platforms.push({
        x: 150 + i * 180 + Math.random() * 80,
        y: h - 120 - Math.random() * 140,
        width: 90 + Math.random() * 60,
        height: 18,
      });
    }

    // Enemies
    const enemyCount = 4 + this.level * 2;
    const types: Enemy["type"][] = ["shadow", "dark-knight", "void-lord"];

    for (let i = 0; i < enemyCount; i++) {
      const x = 300 + i * 220 + Math.random() * 80;
      const type = types[Math.floor(Math.random() * Math.min(types.length, this.level))];

      this.enemies.push({
        x,
        y: h - 80,
        width: type === "void-lord" ? 36 : 32,
        height: type === "void-lord" ? 48 : 44,
        vx: Math.random() > 0.5 ? 2.5 : -2.5,
        type,
        health: type === "void-lord" ? 5 : type === "dark-knight" ? 4 : 3,
        maxHealth: type === "void-lord" ? 5 : type === "dark-knight" ? 4 : 3,
        facing: "left",
        attackTimer: 0,
        patrolLeft: x - 100,
        patrolRight: x + 100,
      });
    }
  }

  setKey(key: keyof typeof this.keys, value: boolean) {
    this.keys[key] = value;
  }

  private gameLoop() {
    if (this.status !== "playing") return;

    this.update();
    this.draw();
    this.animationId = requestAnimationFrame(() => this.gameLoop());
  }

  private update() {
    this.updatePlayer();
    this.updateEnemies();
    this.updateLightBeams();
    this.updateFeathers();
    this.updateCamera();
    this.checkLevelComplete();
    this.emitState();
  }

  private updatePlayer() {
    const p = this.player;

    // Divine energy regeneration
    this.divineRegenTimer++;
    if (this.divineRegenTimer >= 30) {
      this.divineRegenTimer = 0;
      p.divine = Math.min(p.maxDivine, p.divine + 1);
    }

    // Horizontal movement
    const speed = p.angelForm ? ANGEL_SPEED : MOVE_SPEED;
    if (this.keys.left) {
      p.vx = -speed;
      p.facing = "left";
    } else if (this.keys.right) {
      p.vx = speed;
      p.facing = "right";
    } else {
      p.vx = 0;
    }

    // Jump / Flight
    if (this.keys.jump) {
      if (!p.isJumping) {
        p.vy = p.angelForm ? JUMP_FORCE * 1.3 : JUMP_FORCE;
        p.isJumping = true;
      } else if (p.angelForm && p.canFly) {
        // Flight in angel form
        p.vy -= FLIGHT_SPEED;
        p.vy = Math.max(p.vy, -4);
      }
    }

    // Light Attack
    if (this.keys.attack && !p.isAttacking) {
      p.isAttacking = true;
      p.attackTimer = ATTACK_DURATION;
      this.lightAttack();
    }

    // Special: Angel Transformation
    if (this.keys.special && p.divine >= 40 && !p.angelForm) {
      p.divine -= 40;
      p.angelForm = true;
      p.angelFormTimer = 240; // 4 seconds
      p.canFly = true;
      p.width = 38;
      p.height = 50;
      this.spawnFeathers();
    }

    // Update attack timer
    if (p.isAttacking) {
      p.attackTimer--;
      if (p.attackTimer <= 0) {
        p.isAttacking = false;
      }
    }

    // Update angel form
    if (p.angelForm) {
      p.angelFormTimer--;
      if (p.angelFormTimer <= 0) {
        p.angelForm = false;
        p.canFly = false;
        p.width = 32;
        p.height = 46;
      }
    }

    // Update invincibility
    if (p.invincible) {
      p.invincibleTimer--;
      if (p.invincibleTimer <= 0) {
        p.invincible = false;
      }
    }

    // Apply gravity
    p.vy += GRAVITY;

    // Apply velocity
    p.x += p.vx;
    p.y += p.vy;

    // Platform collision
    p.isJumping = true;
    for (const plat of this.platforms) {
      if (this.checkPlatformCollision(p, plat)) {
        if (p.vy > 0) {
          p.y = plat.y - p.height;
          p.vy = 0;
          p.isJumping = false;
        }
      }
    }

    // World bounds
    p.x = Math.max(0, Math.min(this.levelWidth - p.width, p.x));

    // Fall death
    if (p.y > this.canvas.height + 100) {
      p.health = 0;
      this.gameOver();
    }
  }

  private checkPlatformCollision(entity: { x: number; y: number; width: number; height: number; vy?: number }, plat: Platform): boolean {
    return (
      entity.x < plat.x + plat.width &&
      entity.x + entity.width > plat.x &&
      entity.y + entity.height >= plat.y &&
      entity.y + entity.height <= plat.y + plat.height + 10 &&
      (entity.vy === undefined || entity.vy >= 0)
    );
  }

  private lightAttack() {
    const p = this.player;
    const range = p.angelForm ? 70 : 50;
    const attackX = p.facing === "right" ? p.x + p.width : p.x - range;
    const damage = p.angelForm ? 3 : 2;

    // Create light beam effect
    this.lightBeams.push({
      x: p.x + p.width / 2,
      y: p.y + p.height / 2,
      vx: p.facing === "right" ? 14 : -14,
      vy: 0,
      size: p.angelForm ? 20 : 14,
      life: 30,
    });

    // Melee attack
    for (const e of this.enemies) {
      if (
        e.x + e.width > attackX &&
        e.x < attackX + range &&
        Math.abs(e.y - p.y) < 50
      ) {
        e.health -= damage;
        p.divine = Math.min(p.maxDivine, p.divine + 8);
        if (e.health <= 0) {
          this.score += e.type === "void-lord" ? 60 : e.type === "dark-knight" ? 45 : 30;
          const idx = this.enemies.indexOf(e);
          if (idx !== -1) this.enemies.splice(idx, 1);
        }
      }
    }
  }

  private spawnFeathers() {
    for (let i = 0; i < 8; i++) {
      const angle = (Math.PI * 2 * i) / 8;
      this.feathers.push({
        x: this.player.x + this.player.width / 2,
        y: this.player.y + this.player.height / 2,
        vx: Math.cos(angle) * 7,
        vy: Math.sin(angle) * 7,
        life: 50,
      });
    }
  }

  private updateLightBeams() {
    for (let i = this.lightBeams.length - 1; i >= 0; i--) {
      const beam = this.lightBeams[i];
      beam.x += beam.vx;
      beam.y += beam.vy;
      beam.life--;

      if (beam.life <= 0) {
        this.lightBeams.splice(i, 1);
        continue;
      }

      // Hit enemies
      for (let j = this.enemies.length - 1; j >= 0; j--) {
        const e = this.enemies[j];
        if (this.checkBeamHit(beam, e)) {
          e.health -= 1;
          this.lightBeams.splice(i, 1);
          this.player.divine = Math.min(this.player.maxDivine, this.player.divine + 4);
          if (e.health <= 0) {
            this.score += e.type === "void-lord" ? 60 : e.type === "dark-knight" ? 45 : 30;
            this.enemies.splice(j, 1);
          }
          break;
        }
      }
    }
  }

  private checkBeamHit(beam: LightBeam, target: { x: number; y: number; width: number; height: number }): boolean {
    return (
      beam.x > target.x &&
      beam.x < target.x + target.width &&
      beam.y > target.y &&
      beam.y < target.y + target.height
    );
  }

  private updateFeathers() {
    for (let i = this.feathers.length - 1; i >= 0; i--) {
      const feather = this.feathers[i];
      feather.x += feather.vx;
      feather.y += feather.vy;
      feather.life--;

      if (feather.life <= 0) {
        this.feathers.splice(i, 1);
        continue;
      }

      // Hit enemies
      for (let j = this.enemies.length - 1; j >= 0; j--) {
        const e = this.enemies[j];
        if (this.checkFeatherHit(feather, e)) {
          e.health -= 2;
          this.feathers.splice(i, 1);
          if (e.health <= 0) {
            this.score += e.type === "void-lord" ? 60 : e.type === "dark-knight" ? 45 : 30;
            this.enemies.splice(j, 1);
          }
          break;
        }
      }
    }
  }

  private checkFeatherHit(feather: Feather, target: { x: number; y: number; width: number; height: number }): boolean {
    return (
      feather.x > target.x - 10 &&
      feather.x < target.x + target.width + 10 &&
      feather.y > target.y - 10 &&
      feather.y < target.y + target.height + 10
    );
  }

  private updateEnemies() {
    for (const e of this.enemies) {
      // Patrol movement
      e.x += e.vx;
      if (e.x <= e.patrolLeft || e.x >= e.patrolRight) {
        e.vx = -e.vx;
      }
      e.facing = e.vx < 0 ? "left" : "right";

      // Check player collision
      if (!this.player.invincible && this.checkEntityCollision(this.player, e)) {
        const damage = e.type === "void-lord" ? 20 : e.type === "dark-knight" ? 18 : 12;
        this.hitPlayer(damage);
      }
    }
  }

  private checkEntityCollision(a: { x: number; y: number; width: number; height: number }, b: { x: number; y: number; width: number; height: number }): boolean {
    return (
      a.x < b.x + b.width &&
      a.x + a.width > b.x &&
      a.y < b.y + b.height &&
      a.y + a.height > b.y
    );
  }

  private hitPlayer(damage: number) {
    if (this.player.angelForm) {
      damage = Math.floor(damage * 0.5);
    }
    this.player.health -= damage;
    this.player.invincible = true;
    this.player.invincibleTimer = 60;
    this.player.divine = Math.min(this.player.maxDivine, this.player.divine + 15);

    if (this.player.health <= 0) {
      this.gameOver();
    }
  }

  private updateCamera() {
    const targetX = this.player.x - this.canvas.width / 3;
    this.cameraX += (targetX - this.cameraX) * 0.1;
    this.cameraX = Math.max(0, Math.min(this.levelWidth - this.canvas.width, this.cameraX));
  }

  private checkLevelComplete() {
    if (this.enemies.length === 0 && this.player.x > this.levelWidth - 100) {
      this.status = "clear";
      if (this.animationId) {
        cancelAnimationFrame(this.animationId);
      }
      this.emitState();
    }
  }

  nextLevel() {
    this.level++;
    this.player.health = Math.min(this.player.maxHealth, this.player.health + 30);
    this.player.divine = 50;
    this.setupLevel();
    this.status = "playing";
    this.emitState();
    this.gameLoop();
  }

  private gameOver() {
    this.status = "over";
    if (this.animationId) {
      cancelAnimationFrame(this.animationId);
    }
    this.emitState();
  }

  private draw() {
    const ctx = this.ctx;
    const w = this.canvas.width;
    const h = this.canvas.height;

    // Background - heavenly sky
    const gradient = ctx.createLinearGradient(0, 0, 0, h);
    gradient.addColorStop(0, "#87CEEB");
    gradient.addColorStop(0.5, "#B0E0E6");
    gradient.addColorStop(1, "#E0F6FF");
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, w, h);

    // Clouds
    ctx.fillStyle = "rgba(255, 255, 255, 0.6)";
    for (let i = 0; i < 5; i++) {
      const cx = ((i * 200 + Date.now() / 50) % (w + 200)) - 100;
      const cy = 50 + i * 30;
      ctx.beginPath();
      ctx.arc(cx, cy, 30, 0, Math.PI * 2);
      ctx.arc(cx + 25, cy, 35, 0, Math.PI * 2);
      ctx.arc(cx + 50, cy, 30, 0, Math.PI * 2);
      ctx.fill();
    }

    // Floating light particles
    ctx.fillStyle = "rgba(255, 255, 200, 0.7)";
    for (let i = 0; i < 40; i++) {
      const px = (i * 67) % w;
      const py = (i * 41 + Date.now() / 20) % h;
      ctx.beginPath();
      ctx.arc(px, py, 2, 0, Math.PI * 2);
      ctx.fill();
    }

    ctx.save();
    ctx.translate(-this.cameraX, 0);

    // Draw platforms
    for (const plat of this.platforms) {
      this.drawPlatform(plat);
    }

    // Draw enemies
    for (const enemy of this.enemies) {
      this.drawEnemy(enemy);
    }

    // Draw feathers
    for (const feather of this.feathers) {
      this.drawFeather(feather);
    }

    // Draw light beams
    for (const beam of this.lightBeams) {
      this.drawLightBeam(beam);
    }

    // Draw player
    this.drawPlayer();

    ctx.restore();

    // UI overlay
    this.drawUI();
  }

  private drawPlatform(plat: Platform) {
    const ctx = this.ctx;

    if (plat.height === 40) {
      // Ground - golden
      ctx.fillStyle = "#F5DEB3";
      ctx.fillRect(plat.x, plat.y, plat.width, plat.height);
      ctx.fillStyle = "#FFE4B5";
      ctx.fillRect(plat.x, plat.y, plat.width, 5);
    } else {
      // Celestial platforms - cloud-like
      ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
      ctx.fillRect(plat.x, plat.y, plat.width, plat.height);
      ctx.fillStyle = "rgba(230, 230, 250, 0.9)";
      ctx.fillRect(plat.x, plat.y, plat.width, 4);
    }
  }

  private drawPlayer() {
    const ctx = this.ctx;
    const p = this.player;

    if (p.invincible && Math.floor(p.invincibleTimer / 4) % 2 === 0) {
      ctx.globalAlpha = 0.5;
    }

    if (p.angelForm) {
      // Angel form with wings
      ctx.fillStyle = "#ffffff";
      ctx.fillRect(p.x + 7, p.y + 15, p.width - 14, p.height - 15);

      // Head with halo
      ctx.fillStyle = "#ffe4b5";
      ctx.beginPath();
      ctx.arc(p.x + p.width / 2, p.y + 10, 10, 0, Math.PI * 2);
      ctx.fill();

      // Halo
      ctx.strokeStyle = "#ffd700";
      ctx.lineWidth = 2;
      ctx.shadowColor = "#ffd700";
      ctx.shadowBlur = 10;
      ctx.beginPath();
      ctx.arc(p.x + p.width / 2, p.y, 12, 0, Math.PI * 2);
      ctx.stroke();
      ctx.shadowBlur = 0;

      // Eyes
      ctx.fillStyle = "#4a90e2";
      ctx.fillRect(p.x + p.width / 2 - 6, p.y + 9, 3, 3);
      ctx.fillRect(p.x + p.width / 2 + 3, p.y + 9, 3, 3);

      // Wings
      ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
      ctx.shadowColor = "#ffffff";
      ctx.shadowBlur = 15;
      // Left wing
      ctx.beginPath();
      ctx.ellipse(p.x - 8, p.y + 25, 12, 20, -0.3, 0, Math.PI * 2);
      ctx.fill();
      // Right wing
      ctx.beginPath();
      ctx.ellipse(p.x + p.width + 8, p.y + 25, 12, 20, 0.3, 0, Math.PI * 2);
      ctx.fill();
      ctx.shadowBlur = 0;
    } else {
      // Human form
      ctx.fillStyle = "#e0e0e0";
      ctx.fillRect(p.x + 4, p.y + 12, p.width - 8, p.height - 12);

      // Head
      ctx.fillStyle = "#deb887";
      ctx.beginPath();
      ctx.arc(p.x + p.width / 2, p.y + 8, 9, 0, Math.PI * 2);
      ctx.fill();

      // Eyes
      ctx.fillStyle = "#4a90e2";
      ctx.fillRect(p.x + p.width / 2 - 5, p.y + 7, 3, 3);
      ctx.fillRect(p.x + p.width / 2 + 2, p.y + 7, 3, 3);

      // Armor marking
      ctx.strokeStyle = "#b0c4de";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(p.x + p.width / 2, p.y + 25, 6, 0, Math.PI * 2);
      ctx.stroke();
    }

    ctx.globalAlpha = 1;
  }

  private drawEnemy(e: Enemy) {
    const ctx = this.ctx;

    const colors = {
      shadow: "#2f2f2f",
      "dark-knight": "#1a1a2e",
      "void-lord": "#0a0a1a",
    };

    ctx.fillStyle = colors[e.type];
    ctx.fillRect(e.x, e.y, e.width, e.height);

    // Dark aura
    if (e.type === "void-lord") {
      ctx.strokeStyle = "rgba(100, 0, 200, 0.5)";
      ctx.lineWidth = 2;
      ctx.strokeRect(e.x - 2, e.y - 2, e.width + 4, e.height + 4);
    }

    // Health bar
    if (e.health < e.maxHealth) {
      const barWidth = e.width;
      const barHeight = 4;
      const hpPercent = e.health / e.maxHealth;

      ctx.fillStyle = "#333";
      ctx.fillRect(e.x, e.y - 10, barWidth, barHeight);
      ctx.fillStyle = "#e74c3c";
      ctx.fillRect(e.x, e.y - 10, barWidth * hpPercent, barHeight);
    }
  }

  private drawLightBeam(beam: LightBeam) {
    const ctx = this.ctx;

    ctx.save();
    ctx.fillStyle = "rgba(255, 255, 200, 0.9)";
    ctx.shadowColor = "#ffff00";
    ctx.shadowBlur = 20;

    ctx.beginPath();
    ctx.arc(beam.x, beam.y, beam.size / 2, 0, Math.PI * 2);
    ctx.fill();

    // Light rays
    ctx.strokeStyle = "rgba(255, 255, 150, 0.7)";
    ctx.lineWidth = 2;
    for (let i = 0; i < 4; i++) {
      const angle = (Math.PI * 2 * i) / 4;
      ctx.beginPath();
      ctx.moveTo(beam.x, beam.y);
      ctx.lineTo(beam.x + Math.cos(angle) * 10, beam.y + Math.sin(angle) * 10);
      ctx.stroke();
    }

    ctx.restore();
  }

  private drawFeather(feather: Feather) {
    const ctx = this.ctx;

    ctx.save();
    ctx.fillStyle = "rgba(255, 255, 255, 0.9)";
    ctx.shadowColor = "#ffffff";
    ctx.shadowBlur = 10;

    // Feather shape
    ctx.beginPath();
    ctx.ellipse(feather.x, feather.y, 4, 8, Math.atan2(feather.vy, feather.vx), 0, Math.PI * 2);
    ctx.fill();

    ctx.restore();
  }

  private drawUI() {
    const ctx = this.ctx;
    const p = this.player;

    const barWidth = 150;
    const barHeight = 15;
    const x = 20;
    const y = 20;

    // Health bar
    ctx.fillStyle = "rgba(0, 0, 0, 0.5)";
    ctx.fillRect(x, y, barWidth, barHeight);

    const hpPercent = p.health / p.maxHealth;
    const hpColor = hpPercent > 0.5 ? "#00c864" : hpPercent > 0.25 ? "#f39c12" : "#e74c3c";
    ctx.fillStyle = hpColor;
    ctx.fillRect(x, y, barWidth * hpPercent, barHeight);

    ctx.strokeStyle = "#666";
    ctx.lineWidth = 2;
    ctx.strokeRect(x, y, barWidth, barHeight);

    // Divine energy bar
    const divineY = y + barHeight + 5;
    ctx.fillStyle = "rgba(0, 0, 0, 0.5)";
    ctx.fillRect(x, divineY, barWidth, barHeight);

    const divinePercent = p.divine / p.maxDivine;
    ctx.fillStyle = "#87CEEB";
    ctx.fillRect(x, divineY, barWidth * divinePercent, barHeight);

    ctx.strokeStyle = "#4682B4";
    ctx.strokeRect(x, divineY, barWidth, barHeight);

    // Angel form indicator
    if (p.angelForm) {
      ctx.fillStyle = "#ffd700";
      ctx.font = "bold 14px sans-serif";
      ctx.shadowColor = "#ffd700";
      ctx.shadowBlur = 10;
      ctx.fillText("ANGEL FORM", x, divineY + barHeight + 20);
      ctx.shadowBlur = 0;
    }
  }

  destroy() {
    if (this.animationId) {
      cancelAnimationFrame(this.animationId);
    }
  }
}
