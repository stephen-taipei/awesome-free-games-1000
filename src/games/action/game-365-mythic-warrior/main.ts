/**
 * Mythic Warrior Main Entry
 * Game #365
 */
import { MythicWarriorGame } from "./game";
...
const mythDisplay = document.getElementById("myth-display")!;
...
function initGame() {
  game = new MythicWarriorGame(canvas);
  game.resize();
  ...
  btnAttack1.addEventListener("click", () => game.thunderAttack());
  btnAttack2.addEventListener("click", () => game.divineAttack());
  ...
  game.setOnStateChange((state) => {
    ...
    mythDisplay.textContent = state.myth.toString();
    powerDisplay.textContent = state.power.toFixed(1);

    if (state.status === "over") {
      showGameOver(state.myth, state.power);
    }
  });
...
function showGameOver(myth: number, power: number) {
  setTimeout(() => {
    overlay.style.display = "flex";
    overlayTitle.textContent = i18n.t("game.over");
    overlayMsg.textContent = `${i18n.t("game.myth")}: ${myth} | ${i18n.t("game.power")}: ${power.toFixed(1)}x`;
