import { DoubleDragonGame } from './game';
...
function initGame() {
  game = new DoubleDragonGame(canvas);
  game.resize();
  game.setOnStateChange((state) => {
    ...
    else if (state.status === 'cleared') showOverlay(`Stage ${state.stage} Clear!`, `${i18n.t('game.score')}: ${state.score}`, i18n.t('game.nextStage'));
  });
...
startBtn.addEventListener('click', () => {
  ...
  (game as any).status === 'cleared' ? game.nextStage() : game.start();
});

['left', 'right', 'up', 'down', 'punch', 'kick'].forEach(action => {
