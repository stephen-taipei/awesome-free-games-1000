/**
 * Flame Knight Main Entry
 * Game #323
 */
import { FlameKnightGame } from "./game";
...
function initGame() {
  game = new FlameKnightGame(canvas);
  game.resize();
  game.setOnStateChange((state) => {
    scoreDisplay.textContent = state.score.toString();
    heatDisplay.textContent = state.heat.toString();
    waveDisplay.textContent = state.wave.toString();
    if (state.status === "over") showGameOver(state.score);
    else if (state.status === "complete") showWaveComplete(state.score, state.wave);
  });
...
function showWaveComplete(score: number, wave: number) {
  setTimeout(() => {
    ...
    startBtn.onclick = () => { overlay.style.display = "none"; game.nextWave(); };
  }, 500);
}

function startGame() { overlay.style.display = "none"; game.start(); }
