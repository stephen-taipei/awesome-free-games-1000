import { MetalSlugGame } from './game';
...
function initGame() {
  ...
  game.setOnStateChange((state) => {
    ...
    else if (state.status === 'cleared') showOverlay(i18n.t('game.cleared'), `${i18n.t('game.score')}: ${state.score}`, i18n.t('game.restart'));
  });
...
['left', 'right', 'shoot', 'jump', 'grenade', 'crouch'].forEach(action => {
