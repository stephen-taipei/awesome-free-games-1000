...
function initGame() {
  ...
  // Mobile controls
  const setupMobileButton = (btn: HTMLElement, key: "left" | "right" | "jump" | "attack") => {
...
function showLevelClear(score: number, level: number) {
  setTimeout(() => {
    ...
    overlayMsg.textContent = `Level ${level} Complete! Score: ${score}`;
