import { ClawHunterGame } from './game';
...
function initGame() {
  game = new ClawHunterGame(canvas);
  game.resize();
  game.setOnStateChange((state) => {
    ...
    rageFill.style.width = `${state.rage}%`;
    rageFill.classList.toggle('full', state.rage >= 100);
    rageFill.classList.toggle('active', state.rageMode);
    if (state.status === 'over') showOverlay(i18n.t('game.over'), `${i18n.t('game.score')}: ${Math.floor(state.score)}`, i18n.t('game.restart'));
    else if (state.status === 'cleared') showOverlay(`Wave ${state.wave} Clear!`, `${i18n.t('game.score')}: ${Math.floor(state.score)}`, i18n.t('game.nextWave'));
  });
...
startBtn.addEventListener('click', () => {
  ...
  (game as any).status === 'cleared' ? game.nextWave() : game.start();
});

['left', 'right', 'up', 'down', 'attack', 'dash', 'rage'].forEach(action => {
