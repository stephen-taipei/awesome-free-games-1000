/**
 * Witch Hunter Main Entry
 * Game #357
 */
import { WitchHunterGame } from "./game";
...
function initGame() {
  game = new WitchHunterGame(canvas);
  game.resize();
  ...
  // Mobile buttons
  btnUp.addEventListener("touchstart", (e) => { e.preventDefault(); game.setKey("up", true); });
  btnUp.addEventListener("touchend", () => game.setKey("up", false));
  btnDown.addEventListener("touchstart", (e) => { e.preventDefault(); game.setKey("down", true); });
  btnDown.addEventListener("touchend", () => game.setKey("down", false));
  ...
  game.setOnStateChange((state) => {
    ...
    silverDisplay.textContent = state.silver.toString();
